...
export default function SignupForm() {
    ...
    return (
        ...
        <form className="rounded-2xl bg-white h-1/2 p-8 flex flex-col gap-4 min-w-1/2 justify-center items-center" aria-label="signup for an account" onSubmit={handleSignup} noValidate>
            ...
            {/* google button */}
            <button className="py-2 px-5 cursor-pointer shadow-2xs hover:shadow-md rounded-lg border-dark/15 border-2" onClick={handleGoogleSignup}><i className="bi bi-google mr-2"></i>Sign up with Google</button>
            ...
            <div className="relative w-full">
                ...
                {/* toggle button */}
                <button className="absolute right-3 top-1.5 text-2xl" onClick={() => setShowPassword(!showPassword)}>
                    {showPassword ? (
            ...
            {/* submit button */}
            <Button shape="auth" label="Sign Up" className="bg-berry"/>
        </form>
    )
}